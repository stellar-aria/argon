...
auto vectorize_load_interleaved = describe("vectorize_load_interleaved", ${
  it("returns a vectorize_load_interleaved::Iterator when begin() is called", _{
    using element_type = int16_t;
    std::array<element_type, 8> vals;
    auto vec = argon::vectorize_load_interleaved(vals);
    expect(std::is_same_v<decltype(vec.begin()), typename argon::vectorize_load_interleaved<element_type, 2>::iterator>).to_be_true();
  });
  ...
  it("can access all elements of vals", _{
    ...
    for (auto [val_a, val_b] : argon::vectorize_load_interleaved(vals)){
