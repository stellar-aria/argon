...
auto vectorize_store_interleaved = describe("vectorize_store_interleaved", ${
  it("returns a vectorize_store_interleaved::iterator when begin() is called", _{
    ...
    auto vec = argon::vectorize::store_interleaved(vals);
    expect(std::is_same_v<decltype(vec.begin()), typename argon::vectorize::store_interleaved<element_type>::iterator>).to_be_true();
  });
